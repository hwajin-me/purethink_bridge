...
test('custom TCP destination bypasses origin DNS and records client port, traffic and bounded history', async (t) => {
  ...
  const proxy = createTcpProxy({ port: 20622, target: {host:'127.0.0.1',port:upstream.address().port}, lookup() { assert.fail('Must not resolve origin for custom target'); } });
  log.observe(proxy, {port:20622,mode:'custom-tcp'});
  proxy.listen(0, '127.0.0.1'); await once(proxy, 'listening');
  t.after(() => proxy.close());
  const client = net.connect(proxy.address().port, '127.0.0.1');
  await once(client, 'connect');
  const data = once(client, 'data'); client.write('payload');
  assert.equal((await data)[0].toString(), 'payload');
  ...
  assert.equal(closed.bytesRead, 7); assert.equal(closed.remoteAddress, '127.0.0.1');
  assert.equal(closed.bytesWritten, 7);
