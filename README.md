...
위 `Client ID`는 Bridge가 내부 MQTT 서버에 접속할 때 쓰는 ID입니다.

대시보드의 **기기 등록 → 기기 추가**에서 여러 기기를 등록할 수 있습니다.

- **기기 ID**: `/things/<기기 ID>/...` 토픽에 쓰이는 실제 ID. `DIV` 접두사 없이도 등록할 수 있습니다.
- **MQTT 접속 Client ID**: 실제 연결 ID가 기기 ID와 다를 때 입력합니다. 화면의 현재 MQTT 접속 ID 목록을 참고하세요. 비우면 기기 ID와 같은 값으로 저장합니다.
- **이름**: 거실·사무실 등 구분용 이름입니다.

**기기 목록 저장**을 누르면 재시작 없이 적용되며 설정 파일에 보존됩니다. 등록은 기기 자체의 ID나 메시지 토픽을 변경하지 않습니다. 각 기기의 연결 상태·실제 기기 ID·접속 Client ID·최근 토픽·RX/TX를 별도로 표시합니다. 등록된 기기의 `/things/<기기 ID>/#`는 기기가 오프라인이어도 제조사 MQTT 구독을 유지합니다.

수동 등록하지 않은 `DIV01-<식별자>` 연결은 기존처럼 자동 인식합니다. 그 밖의 접속은 직접 등록하기 전까지 일반 클라이언트로 취급하여 다른 기기의 ID나 상태를 덮어쓰지 않습니다. MQTT 토픽 구분자(`/`), 와일드카드(`+`, `#`), 공백은 기기 ID에 사용할 수 없으며 중복된 기기 ID나 접속 Client ID 등록은 거부합니다.

**MQTT 연결 탐지**는 Bridge를 지나는 클라이언트 → 서버 스트림의 원본 CONNECT 패킷에서 Client ID를 읽습니다. MQTT 3.1/3.1.1/5와 분할 수신을 지원하며 접속 IP·포트·시각·연결 종료 여부를 함께 표시합니다. PUBLISH·SUBSCRIBE의 `/things/<ID>/...`에서 추출한 **기기 ID 후보**는 접속 Client ID와 별도로 표시합니다. 후보가 여러 개면 선택 후 **등록 입력란에 추가 → 기기 목록 저장**으로 등록하세요. 이는 관찰된 값이며 앱이 여러 기기 토픽을 구독할 수도 있으므로 자동으로 기기로 등록하지 않습니다.

- 기본 8885 MQTT/TLS: TLS 해제 후 원본 CONNECT를 관찰합니다. 빈 Client ID도 원본 그대로 표시하며, 브로커가 생성한 ID로 대체하지 않습니다.
