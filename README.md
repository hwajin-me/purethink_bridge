...
MQTT 접속 Client ID는 Bridge가 내부적으로 처리합니다. 기기 등록이나 상태 판단에 사용하지 않으며 별도로 입력할 필요가 없습니다. 기존 내부 브로커 접속 설정은 보존합니다.

대시보드의 **기기 등록 → 기기 추가**에서 여러 기기를 등록할 수 있습니다.

- **기기 ID**: `/things/<기기 ID>/...` 토픽에 쓰이는 실제 ID. `DIV` 접두사 없이도 등록할 수 있습니다.
- **이름**: 거실·사무실 등 구분용 이름입니다.

**기기 목록 저장**을 누르면 재시작 없이 적용되며 설정 파일에 보존됩니다. 등록은 기기 자체의 ID나 메시지 토픽을 변경하지 않습니다. 각 기기의 연결 상태·실제 기기 ID·최근 토픽·RX/TX를 별도로 표시합니다. 등록된 기기의 `/things/<기기 ID>/#`는 기기가 오프라인이어도 제조사 MQTT 구독을 유지합니다.

접속 Client ID 대신 `/things/<기기 ID>/...` 토픽으로 기기를 구분하며 모델 접두사는 제한하지 않습니다. 구독 토픽은 기기 ID 탐지·제조사 구독에만 사용하고, 실제 `/shadow` 발행이 확인된 연결만 직접 연결로 표시합니다. 빈 Client ID, 변경되는 Client ID, 한 연결의 여러 기기 토픽도 지원합니다. 기존 기기별 `clientId` 매핑은 로딩 시 자동 제거합니다. MQTT 토픽 구분자(`/`), 와일드카드(`+`, `#`), 공백은 기기 ID에 사용할 수 없으며 중복된 기기 ID 등록은 거부합니다.

기기가 Bridge에 직접 연결하지 않고 제조사 서버에 연결된 경우에도, 등록된 기기의 `/things/<기기 ID>/shadow`를 제조사 MQTT에서 새로 받으면 **제조사 경유 · 최근 90초 내 상태 수신**으로 온라인 표시하고 Last Seen·Last Topic·RX를 갱신합니다. 직접 연결이 있으면 **Bridge 직접 연결**을 우선 표시합니다. 직접 연결 없이 90초간 새 상태를 받지 못하면 offline이 됩니다. 구독만 하는 앱, retained 상태 메시지, `/command` 메시지, Bridge가 되받은 에코는 이 활동 시간을 갱신하지 않습니다. 제조사 경유 온라인은 로컬 직접 제어 가능 여부와 구분하므로 직접 연결이 확인되지 않으면 Local control은 available로 바뀌지 않습니다.

**MQTT 연결 탐지**는 Bridge를 지나는 클라이언트 → 서버 스트림에서 기기 토픽을 관찰합니다. MQTT 3.1/3.1.1/5와 분할 수신을 지원하며 접속 IP·포트·시각·연결 종료 여부를 함께 표시합니다. PUBLISH·SUBSCRIBE의 `/things/<ID>/...`에서 추출한 **기기 ID 후보**를 표시합니다. 접속 Client ID가 비어 있어도 후보를 등록할 수 있습니다. 후보가 여러 개면 선택 후 **등록 입력란에 추가 → 기기 목록 저장**으로 등록하세요. 기기 이름과 영구 등록 목록은 저장 버튼으로 관리합니다. 구독에서 찾은 후보만으로 실제 기기가 온라인이라고 판단하지 않습니다.

- 기본 8885 MQTT/TLS: TLS 해제 후 원본 CONNECT를 관찰합니다. 접속 Client ID는 내부 진단 정보로만 남기며 기기 매핑에 사용하지 않습니다.
- TCP 전달 포트: 평문 MQTT이면 관찰하고, TLS passthrough이면 암호화로 인해 MQTT 여부와 ID를 확인할 수 없다고 표시합니다. 암호화된 연결의 ID를 확인하려면 클라이언트가 신뢰하는 인증서와 명시적 TLS 종료 프록시 구성이 필요합니다. 탐지 기능이 인증서나 전달 방식을 자동 변경하지는 않습니다.
- 사용자 포트 서비스의 stream TCP/TLS 프록시: 평문 또는 TLS 해제 후 관찰합니다. HTTP/WebSocket MQTT는 이 탐지기의 지원 대상이 아닙니다.

탐지는 전달 바이트를 변경하지 않습니다. 화면의 연결 기록은 최근 100개, 연결당 토픽·기기 ID 후보는 최대 20개이며 패킷 메타데이터 분석 버퍼는 최대 128 KiB입니다. 탐지 이벤트는 기존 `data/logs/access.jsonl`(LXC: `/var/lib/purethink-bridge/logs/access.jsonl`)의 크기 제한·회전 정책을 사용합니다. 탐지 로그에는 CONNECT의 username/password나 메시지 본문을 기록하지 않습니다. 이미 연결 중인 세션의 CONNECT는 소급해서 읽을 수 없으므로 적용 후 기기 또는 앱을 다시 연결하세요.
...
### 웹에서 Custom Firmware Build

대시보드의 **Custom Firmware Build · DIV01**에서 펌웨어를 생성할 수 있습니다.

1. **Prepare / Retry DIV01 Firmware**로 SHA-256이 검증된 `1630` 원본을 준비합니다. 이미 준비된 원본은 인터넷 없이 재사용합니다.
2. 출력 버전을 `ver.220706.NNNN_DIV01` 형식으로 지정합니다. `NNNN`은 1633보다 커야 하며 기본값은 1634입니다.
3. 필요하면 공개 Root CA PEM 파일을 선택하거나 **서버에 설정된 Root CA 동봉**을 선택합니다. 유효기간 내의 자체 서명 CA 인증서 하나만 허용하며 개인키는 받지 않습니다.
4. TLS 검증 우회·수동 플래시 위험 확인 후 **Build Firmware**를 누릅니다.
5. `.bin`, `.manifest.json`, 선택한 `.root-ca.crt`를 각각 다운로드합니다. 명세에 원본/결과 SHA-256, 크기, ESP8266 체크섬, CA 지문을 기록합니다.

이 기능은 소스 컴파일이 아니라 검증된 DIV01 바이너리의 TLS 검증 우회 패치, 동일 길이 버전 문자열 변경, 체크섬 재계산입니다. 사용자 지정 버전은 실제 기기에서 검증되지 않았습니다. 빌드 결과는 다운로드로만 제공하며 서버 파일·기존 1633 OTA 광고를 변경하거나 자동 플래시하지 않습니다. 페이지를 새로고침하면 다운로드 링크가 사라지므로 필요한 파일을 저장하세요.

**Root CA는 별도 동봉 파일입니다. 펌웨어 내부 신뢰 저장소에 포함되지 않으며 장치의 CA 검증을 활성화하지 않습니다.** 기존 패치는 TLS 검증을 우회합니다. 실제 CA 신뢰를 적용하려면 원본 펌웨어의 신뢰 저장소 및 TLS 호출 경로를 추가 분석하거나 빌드 가능한 소스가 필요합니다. 확인되지 않은 영역에 인증서를 삽입하지 않습니다.

관리 포트의 `POST /api/firmware/build`는 다음 JSON을 받습니다. 성공 시 `filename`, `firmwareBase64`, `manifest`, `rootCaPem`을 반환합니다. 원본이 준비되지 않으면 409, 잘못된 옵션은 400을 반환합니다. CA PEM은 최대 16 KiB입니다.

```json
{
  "version": "ver.220706.1634_DIV01",
  "tlsMode": "bypass",
  "rootCaPem": ""
}
```

#### Root CA 검증과 기기 접속 호스트 변경

웹 빌드 폼의 **Root CA 검증** 버튼은 파일 또는 서버에 설정된 공개 인증서의 PEM 형식, Basic Constraints `CA:TRUE`, 발급자/주체 일치, 공개키를 이용한 자체 서명, 유효기간을 검사합니다. Key Usage가 있으면 `keyCertSign`을 요구합니다. 서버 인증서, 중간 CA, 변조된 서명, 만료/미래 인증서, 개인키가 섞인 파일은 거부합니다. 빌드 요청에서도 같은 검증을 다시 수행합니다.

인증서가 유효한 자체 서명 CA라고 해서 의도한 소유자의 CA임이 증명되는 것은 아닙니다. 신뢰하는 별도 경로에서 얻은 SHA-256 지문을 **Root CA SHA-256 지문**에 입력하면 일치 여부까지 검사하며 불일치하면 빌드를 거부합니다. 지문을 생략하면 화면과 명세에 소유자 신뢰 미확인 상태를 표시합니다. 이 검사는 시스템의 공개 신뢰 목록, 폐기 상태 또는 Bridge 서버 인증서와의 체인 연결을 확인하는 기능은 아닙니다. CA 파일은 여전히 별도 동봉되며 장치에 내장되지 않습니다.

**기기 접속 호스트**는 최대 **15 ASCII 바이트**의 DNS 호스트명 또는 IPv4 주소를 받습니다. 기본값은 `dapt.iptime.org`이며 `bridge.lan`, `192.168.0.4` 등으로 바꿀 수 있습니다. URL 스킴·포트·경로·IPv6·유니코드 도메인·15바이트 초과 값은 거부합니다. 사용할 이름이 Bridge IP로 해석되도록 내부 DNS를 설정하세요. 이 입력은 생성되는 펌웨어에만 적용되며 Bridge의 원본 프록시 목적지, 인증서 SAN, OTA 광고 Hostname을 변경하지 않습니다.

SHA-256이 검증된 `ver.220706.1630_DIV01.bin`에서 확인한 문자열과 파일 오프셋:

| 문자열 | 파일 오프셋 | 문자열 시작 주소 | 주소 참조 위치 |
| --- | --- | --- | --- |
| `http://dapt.iptime.org:6002/atmospheric/device?mac=%d` | `0x7bcfb` | `0x3ffe8c4f` | `0x23d0` |
| `http://dapt.iptime.org:6002/device/fwVersion?mac=%d` | `0x7bd35` | `0x3ffe8c89` | `0x249c` |
| `dapt.iptime.org` | `0x7c303` | `0x3ffe9257` | `0x7b5d8` |

원본의 해당 문자열과 NUL 종료를 확인한 후 동일 위치에서 호스트를 교체합니다. HTTP URL의 나머지 경로를 함께 이동하고 남는 공간을 0으로 채워 문자열 시작 주소와 전체 파일 크기를 유지합니다. ESP8266 세그먼트 체크섬을 다시 계산하며 명세에 호스트와 수정 오프셋을 기록합니다. 다른 문자열(`gogotech.iptime.org`, NTP 등)은 변경하지 않습니다. 실제 기기에서 접속·OTA가 동작하는지는 별도 검증이 필요합니다.

`POST /api/firmware/validate-root-ca`는 `{ "rootCaPem": "...", "expectedRootCaFingerprint": "..." }`를 받아 검증 결과와 인증서 정보를 반환합니다. 오류는 400입니다. `POST /api/firmware/build`에도 `hostname`, `expectedRootCaFingerprint`를 선택적으로 지정할 수 있습니다.
...
```json
{
  ...
  "hostname": "bridge.lan",
  "rootCaPem": "",
  "expectedRootCaFingerprint": ""
}
```

CA 용도 검사는 [RFC 5280 Basic Constraints / Key Usage](https://www.rfc-editor.org/rfc/rfc5280.html#section-4.2.1.9)를 기준으로 합니다.
